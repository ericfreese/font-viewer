  ...
  attached() {
    ...
    this.fontViewer.getAvailableCharacters((availableCharacters) =>
      each(availableCharacters, (c) =>
        container.append(
          `<div class=\"font-glyph\">&#x${c.toString(16)};<div class=\"font-glyph-id\">U+${c.toString(16)}</div></div>`
        )
      )
    )

    this.style.append(
      `@font-face { font-family: \"${this.fontViewer.getUri()}\"; src: url('${this.fontViewer.getUri()}'); }`
    )
    this.container.css({ "font-family": `\"${this.fontViewer.getUri()}\"` })
  }
